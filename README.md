...
现象：`This chip is ESP32-S3, not ESP32`。

处理：使用 `barometer_node_s3/barometer_base_s3`，不要用 `barometer_node/barometer_base`。
...
确认使用的是 S3 环境（已含 `ARDUINO_USB_CDC_ON_BOOT=1`）。

### 10.4 只有 BAROT 没有 BAROD
...
colcon build --symlink-install
source install/setup.bash
ros2 run serial_to_ros2 esp32_serial_baro --ros-args -p serial_port:=/dev/ttyACM0 -p output_mode:=self-relative
```
