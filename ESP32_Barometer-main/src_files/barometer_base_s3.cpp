...
#define DATA_COLLECTION_INTERVAL 1000           // 1000 milliseconds
...
#define IP_PRINT_INTERVAL (5 * 1000)            // print local IP every 5 seconds
...
int probeI2CAddress(uint8_t addr);  // Probe I2C address and return Wire error code
void logBarometerI2CDiagnostics();  // Probe BMP390 candidate addresses (0x76/0x77)
...
void setup()
{
    ...
    // Initialize barometer FIRST and keep retrying instead of halting forever.
    logBarometerI2CDiagnostics();
    while (!barometer.begin())
    {
        Serial.println("ERROR: Sensor initialization failed. Check wiring/power/CSB/address.");
        logBarometerI2CDiagnostics();
        delay(2000);
    }
    Serial.printf("INFO: Sensor initialized at I2C address 0x%02X\n", barometer.getAddress());
    ...
    while (!connectWiFi())
    {
        Serial.println("WARNING: WiFi connect failed, retrying in 2s...");
        delay(2000);
    }
    ...
    // Initial Time Sync
    while (!requestTimeSync())
    {
        Serial.println("WARNING: Initial time sync failed. Retrying in 2s...");
        delay(2000);
    }
...
void loop()
{
    ...
    // Print local IP every 5 seconds for easy debugging
    if (currentMillis - lastIpPrintTime >= IP_PRINT_INTERVAL)
    {
        lastIpPrintTime = currentMillis;
        if (WiFi.status() == WL_CONNECTED)
        {
            Serial.println("INFO: ESP32 local IP: " + WiFi.localIP().toString());
        }
        else
        {
            Serial.println("INFO: ESP32 local IP: DISCONNECTED");
        }
    }
...
int probeI2CAddress(uint8_t addr)
{
    Wire.beginTransmission(addr);
    return Wire.endTransmission();
}

void logBarometerI2CDiagnostics()
{
    const uint8_t addrs[] = {BARO_I2C_ADDR_PRIMARY, BARO_I2C_ADDR_SECONDARY};
    for (uint8_t addr : addrs)
    {
        int err = probeI2CAddress(addr);
        Serial.printf("INFO: I2C probe addr=0x%02X result=%d (%s)\n",
                      addr,
                      err,
                      (err == 0 ? "ACK" : "NO_ACK"));
    }
...
bool connectWiFi()
{
    ...
    if (wifiClient.connectAuto())
    {
        ...
        Serial.println("INFO: WiFi connected. IP: " + wifi_status.ipAddress);
...
{   
    wifi_status = wifiClient.getWifiStatus();
    if (!wifi_status.isConnected || WiFi.status() != WL_CONNECTED)
    {
